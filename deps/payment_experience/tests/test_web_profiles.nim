...
suite "web_profiles serialization":
  test "round-trips WebProfile":
    ...
    check openjson.toJson(openjson.fromJson(openjson.toJson(obj), payment_experience.WebProfile)) == openjson.toJson(obj)

  test "round-trips Error":
    let obj = newError()
    check openjson.toJson(openjson.fromJson(openjson.toJson(obj), payment_experience.Error)) == openjson.toJson(obj)
