...
suite "verify_webhook_signature serialization":
  test "round-trips VerifyWebhookSignatureResponse":
    ...
    check openjson.toJson(openjson.fromJson(openjson.toJson(obj), webhooks.VerifyWebhookSignatureResponse)) == openjson.toJson(obj)

  test "round-trips VerifyWebhookSignature":
    let obj = newVerifyWebhookSignature()
    check openjson.toJson(openjson.fromJson(openjson.toJson(obj), webhooks.VerifyWebhookSignature)) == openjson.toJson(obj)
