...
suite "payment_tokens serialization":
  test "round-trips PaymentTokenResponse":
    ...
    check openjson.toJson(openjson.fromJson(openjson.toJson(obj), payment_tokens.PaymentTokenResponse)) == openjson.toJson(obj)

  test "round-trips PaymentTokenRequest":
    let obj = newPaymentTokenRequest()
    check openjson.toJson(openjson.fromJson(openjson.toJson(obj), payment_tokens.PaymentTokenRequest)) == openjson.toJson(obj)

  test "round-trips CustomerVaultPaymentTokensResponse":
    let obj = newCustomerVaultPaymentTokensResponse()
    check openjson.toJson(openjson.fromJson(openjson.toJson(obj), payment_tokens.CustomerVaultPaymentTokensResponse)) == openjson.toJson(obj)
